 ...
 */
 ...
 static size_t generate_3x3_hints(screen_t scr, struct hint *hints)
 {
	 int sw, sh;
	 int w, h;
	 int i, j;
	 ...
	 const char *chars = config_get("hint3x3_chars");
	 const int grid_sz = 3; /* Always 3x3 */

	 get_hint_size(scr, &w, &h);
	 platform->screen_get_dimensions(scr, &sw, &sh);

	 const int colgap = sw / grid_sz - w;
	 const int rowgap = sh / grid_sz - h;

	 const int x_offset = (sw - grid_sz * w - (grid_sz - 1) * colgap) / 2;
	 const int y_offset = (sh - grid_sz * h - (grid_sz - 1) * rowgap) / 2;
 ...
 int hint3x3_mode()
 {
	 ...
	 nr_hints = generate_3x3_hints(scr, hints);
